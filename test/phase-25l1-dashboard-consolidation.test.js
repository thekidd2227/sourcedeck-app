// Phase 25L-1 · Dashboard consolidation test
// ──────────────────────────────────────────────────────────────────────
...
console.log('Phase 25L-1 · Dashboard consolidation');
...
const cards = [
  ...
  { id: 'deal-workspace',      label: 'Deal Workspace',        target: 'dealwork' },
...
// ── Launchpad surfaces removed-from-active-nav summaries ─────────────
// Overdue, Today's Tasks, Pipeline, Reports, and Deal Workspace must
// each be reachable from a Dashboard launchpad card.
const summarizedDestinations = ['overdue','dailyops','pipeline','proof','dealwork'];
summarizedDestinations.forEach(function(target){
  const cardOpenRe = new RegExp('data-dash-card="[^"]+"[\\s\\S]{0,1200}openTab\\(\'' + target + '\'\\)');
  assert(cardOpenRe.test(html),
    'Dashboard launchpad surfaces a card routing to openTab(\'' + target + '\')');
});

console.log(process.exitCode ? 'Phase 25L-1 · Dashboard consolidation: FAILED' : 'Phase 25L-1 · Dashboard consolidation: OK');
