 ...
 *
 * Asserts the Fetch Description flow exists in the renderer, is wired across
 ...
 * locally, and can be sent to the Solicitation Workspace.
 *
...
test('Fetch Description button + handler exist (gated on descriptionLink)', () => {
  assert.ok(/gcW25FetchDescription\(/.test(HTML), 'gcW25FetchDescription handler present');
  assert.ok(/Fetch Description/.test(HTML), 'Fetch Description label present');
  assert.ok(/o\.descriptionLink/.test(HTML), 'fetch button gated on descriptionLink');
});
...
test('fetched description is stored locally + can be used in workspace', () => {
  ...
  assert.ok(/Use description in Solicitation Workspace/.test(HTML), 'use-in-workspace label present');
});
