...
**Date:** 2026-06-25  
**Branch:** `fix/solicitation-incident-complete`  
...
The investigation confirmed an architectural reason this state can occur: summary metadata may fall back to the saved SAM.gov opportunity record, while detailed sections require usable text from uploaded solicitation files. An unreadable, image-only, or unsupported PDF can therefore leave the section classifier with little or no content while the summary still appears populated.
...
Draft PR #162 added a structured summary and Explain actions but introduced several data-integrity and renderer defects. This branch corrects those defects without broad extraction rewrites.
...
1. uploaded PDF yields no usable text or is marked `ocr_required`;
...
The summary and explain service is:

```text
services/govcon/solicitation-summarize.js
```

The renderer module is:
...
The previous summary frequently exposed field names without exact source records, and one date source collector discarded its temporary Set.

Summary areas and section explanations now return:
...
when present in the extraction record.
...
An `ocr_required` package is surfaced as extraction failure rather than being treated as a successful complete extraction.
...
- `services/govcon/solicitation-summarize.js`
  ...
  - section explain based on actual extracted content.

- `app/renderer/features/solicitation-center/summarize-and-explain.js`
  - non-destructive explanation panels;
  - mismatch/unbound failure copy;
...
- `test/phase-25ar-solicitation-summarize-and-extraction-bugs.test.js`
  ...
  - contact/CLIN/compliance serialization;
  - source provenance;
  - OCR-required status;
...
6. structured pricing/CLIN rows;
...
12. section explanation from actual extracted text;
13. empty alias `not_found` behavior;
14. no direct Electron or `ipcRenderer` access;
15. original section text remains unchanged after Explain;
16. separate explanation panel creation.
...
- OCR confidence and provider/method provenance;
...
GitHub CI results should be attached to the pull request. Do not declare the exact production PDF failure closed without testing the affected source file.
...
The confirmed summary/explain defects are corrected on `fix/solicitation-incident-complete`.

The exact production document-extraction failure remains open pending the affected solicitation file. The application now distinguishes and exposes failed or OCR-required processing instead of presenting a misleading complete summary state.